...
namespace WindowsFormsApp1.Desgin
{
    public partial class addBom : Form
    {
        ...
        {
            string str = "select id,materialsId as 物料编码,unitNumber as 库存数量,materialsName as 物料名称 ,specification as 规格型号,stockName as 仓库名称 from MaterialStock where materialsName like '%"+txtWlmc.Text+"%'and specification like '%"+txtGgxh.Text+"%'and stockName like '%"+comboBox1.Text+"%'";
        ...
        {
            string str2 = "select id,wlbm as 物料编码,kc as 库存数量,wlmc as 物料名称 ,ggxh as 规格型号,ckmc as 仓库名称 from Bom where [user] = '" + User + "' and state = 'Y'";
        ...
        private void dataGridView1_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Return)
            {
                ...
                for (int i = 0; i < dataGridView1.SelectedRows.Count; i++)
                {
                    ...
                    sqlCommand.CommandText = "insert into [dbo].[Bom] ([iid],[wlbm],[wlmc],[ggxh],[kc],[ckmc],[user],[state]) VALUES ('"+iid+"','"+wlbm+"','"+wlmc+"','"+ggxh+"','"+kcsl+"','"+ckmc+"','"+User+"','Y')";
                ...
                using (SqlCommand sqlCommand = new SqlCommand(str4, sqlConnection))
                {
                    using (SqlDataReader sqlDataReader = sqlCommand.ExecuteReader())
                    {
                        ...
                        if (hasRows)
                        {
                            ...
                            {
                                "insert into [dbo].[DesginBom] ([orderid],[contractid],[date],[desgin],[company],[project],[color],[product],[sfyl],[meters],[clid],[clmc],[clgg],[cllb],[ccpf],[zs],[bz],[status],[examine],[examine1],[size],[state]) VALUES ('"+text3+"','"+text5+"','"+text4+"','"+User+"','"+text6+"','"+text7+"','"+text9+"','"+text8+"','"+text10+"','"+num2+"','"+text11+"','"+text12+"','"+text13+"','"+text14+"','"+text+"','"+num+"','"+bz+"','未知','未审核','未审核','"+cc+"','N')"
                            });
