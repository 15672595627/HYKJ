...
{
    public partial class CGXBudgetList : Form
    {
        public CGXBudgetList()
        {
            InitializeComponent();
        }
        ...
        {
            string str = "select id,date as 月份 , employee as 员工人数 ,monthlySigningAmountPerEmployee as 职员人均月签单额 ,signingTarget as 签单目标 ,shippingTarget as 出货目标 ,paymentTarget as 回款目标 ,deposit as 定金 ,currentMonthShipmentReceipt as 本月出货收款,collectionEarlyArrears as 收前期欠款 ,refundAccounts as 退过账款,outputValueTarget as 产值目标 ,procurementPlanning  as 采购计划,productionMaterialProcurementPlan as 生产用物料采购计划,equipmentProcurementPlan as 设备采购计划,otherProcurementPlans as 其他采购计划 ,payPriceGoods as 支付货款 ,state as 状态 from ZJBcgxys where date like '%" + dateTimePicker1.Text.Trim()+ "%' and state = 1 order by date desc";
            ...
            for (int i = 0; i < dataGridView1.Rows.Count; i++)
            {
                ...
                sum11 += Convert.ToInt32(dataGridView1.Rows[i].Cells[12].Value);
            ...
            string[] row = { "1","合计" , ssum1, ssum2, ssum3, ssum4, ssum5, ssum6, ssum7, ssum8, ssum9, ssum10, ssum11, ssum12, ssum13, ssum14, ssum15 };
            ((DataTable)dataGridView1.DataSource).Rows.Add(row);
        }

        private void CGXBudgetList_Load(object sender, EventArgs e)
        {
            asc.controllInitializeSize(this);
        }

        private void CGXBudgetList_SizeChanged(object sender, EventArgs e)
        {
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dataGridView1.SelectedRows.Count; i++)
            {
                ...
                cmd.CommandText = "update ZJBcgxys set  state = -1 where id = '" + id + "'";
