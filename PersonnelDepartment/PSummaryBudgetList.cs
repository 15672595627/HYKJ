...
{
    public partial class PSummaryBudgetList : Form
    {
        public PSummaryBudgetList()
        {
            InitializeComponent();
        }
        ...
        private void PSummaryBudgetList_Load(object sender, EventArgs e)
        {
            asc.controllInitializeSize(this);
        }

        private void PSummaryBudgetList_SizeChanged(object sender, EventArgs e)
        {
        ...
        {
            string str = "select [id], [date] as 月份,[wages] as 工资, [awardRaising] as 提奖, [socialSecurity] as 社保, [accumulationFund] as 公积金, [boardExpenses ] as 伙食费, [benefit] as 福利费, [commercialInsurance] as 商业保险, [educationFund] as 教育基金, [weldingExternalProcessing] as 焊接外加工, [processingFee] as加工费, [officeExpenses] as 办公费, [communicationFee] as 通讯费, [travelExpenses] as 差旅费, [automobileExpenses] as 汽车开支, [lowValueConsumables] as 低值易耗品, [waterAndElectricityExpenses] as 水电费, [gasCost] as 燃气费, [freight] as 运费, [PromotionFee] as 推广费, [costOfOperation] as 业务费, [entertainmentExpenses] as 招待费, [rent] as 房租, [maintenanceCost] as 维修费, [constructionSiteExpenses] as 工地开支, [sample] as 样品, [supplementaryOrde] as 补单, [recruitmentFees] as 招聘费, [interestExpense] as 利息支出, [commission] as 手续费, [taxes] as 税金, [other] as 其他, [state] as 状态 from RSBBudget where date like '%" + dateTimePicker1.Text.Trim() + "%' and state = 1 order by date desc";
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dataGridView1.SelectedRows.Count; i++)
            {
                ...
                cmd.CommandText = "update RSBBudget set  state = -1 where id = '" + id + "'";
