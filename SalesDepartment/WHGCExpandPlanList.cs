...
{
    public partial class WHGCExpandPlanList : Form
    {
        public WHGCExpandPlanList()
        {
            InitializeComponent();
        }
        ...
        private void WHGCExpandPlanList_Load(object sender, EventArgs e)
        {
            asc.controllInitializeSize(this);
        }

        private void WHGCExpandPlanList_SizeChanged(object sender, EventArgs e)
        {
        ...
        {
            string str = "select id,date as 时间 , region as 区域 ,province as 省份 ,market as 市 ,county as 区县 ,number as 人数 ,yuefen as 人员到位月份 ,target1 as 签单目标,target2 as 收货目标 ,remark as 备注,state as 状态 from SalesExpandPlan where date like '%" + dateTimePicker1.Text.Trim() + "%' and state = 1 and company = '武汉工程' order by date desc";
