...
{
    public partial class WHGCFixedAssetBudgetList : Form
    {
        public WHGCFixedAssetBudgetList()
        {
            InitializeComponent();
        }
        int cot;
        ...
        DataTable dt;
        SqlDataAdapter da;
        private void WHGCFixedAssetBudgetList_Load(object sender, EventArgs e)
        {
            asc.controllInitializeSize(this);
        }

        private void WHGCFixedAssetBudgetList_SizeChanged(object sender, EventArgs e)
        {
        ...
        {
            string str = "select [id],[apartment] as 部门,[name] as 名称,[purpose] as 用途,[price] as 单价,[number] as 数量,[date] as 月份, [state] as 状态 from SalesFixedAssetBudget where date like '%" + dateTimePicker1.Text.Trim() + "%' and state = 1 and company = '武汉工程' order by date desc";
            ...
            {
                sum1 += Convert.ToDecimal(dataGridView1.Rows[i].Cells["单价"].Value);
                sum2 += Convert.ToInt32(dataGridView1.Rows[i].Cells["数量"].Value);
            }
            string ssum1 = sum1.ToString();
            string ssum2 = sum2.ToString();
            string[] row = { "1", "合计", "", "", ssum1, ssum2, "" };
        ...
        private void dataGridView1_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            WHGCUpdateFixedAssetBudget updateCGXBBudget = new WHGCUpdateFixedAssetBudget();
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dataGridView1.SelectedRows.Count; i++)
            {
                ...
                cmd.CommandText = "update SalesFixedAssetBudget set state = -1 where id = '" + id + "'";
