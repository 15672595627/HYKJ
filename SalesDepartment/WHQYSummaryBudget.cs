...
{
    public partial class WHQYSummaryBudget : Form
    {
        public WHQYSummaryBudget()
        {
            InitializeComponent();
        }
        ...
        private void WHQYSummaryBudget_Load(object sender, EventArgs e)
        {
            asc.controllInitializeSize(this);
        }

        private void WHQYSummaryBudget_SizeChanged(object sender, EventArgs e)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            con.Open();
            string findSj = "select id ,date as 时间 ,state as 状态 from SalesBudget where company = '武汉区域'";
            ...
            for (int i = 0; i < dataGridView1.Rows.Count - 1; i++)
            {
                ...
                if (rq == date && zt == 1)
                {
                    ...
                    {
                        WHQYUpdateSummaryBudget updateCGXBBudget = new WHQYUpdateSummaryBudget();
                ...
                else
                {
                    ...
                    cmd.Connection = con;
                    cmd.CommandText = "INSERT INTO [dbo].[SalesBudget]( [wages], [awardRaising], [socialSecurity], [accumulationFund], [boardExpenses ], [benefit], [commercialInsurance], [educationFund], [weldingExternalProcessing], [processingFee], [officeExpenses], [communicationFee], [travelExpenses], [automobileExpenses], [lowValueConsumables], [waterAndElectricityExpenses], [gasCost], [freight], [PromotionFee], [costOfOperation], [entertainmentExpenses], [rent], [maintenanceCost], [constructionSiteExpenses], [sample], [supplementaryOrde], [recruitmentFees], [interestExpense], [commission], [taxes], [other],[date],[state],[company]) VALUES ('" + gz + "', '" + jt + "', '" + sb + "', '" + gjj + "', '" + hsf + "', '" + flf + "', '" + sybx + "', '" + jyjj + "', '" + hjwjg + "', '" + jgf + "', '" + bgf + "', '" + txf + "', '" + clf + "', '" + qckz + "', '" + dzyhp + "', '" + sdf + "', '" + rqf + "', '" + yf + "', '" + tgf + "', '" + ywf + "', '" + zdf + "', '" + fz + "', '" + wxf + "', '" + gdkz + "', '" + yp + "', '" + bd + "', '" + zpf + "', '" + lxzc + "', '" + sxf + "', '" + sj + "', '" + qt + "','" + date + "',1,'武汉区域')";
