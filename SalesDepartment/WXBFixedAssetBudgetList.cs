...
{
    public partial class WXBFixedAssetBudgetList : Form
    {
        public WXBFixedAssetBudgetList()
        {
            InitializeComponent();
        }
        ...
        private void WXBFixedAssetBudgetList_Load(object sender, EventArgs e)
        {
            asc.controllInitializeSize(this);
        }

        private void WXBFixedAssetBudgetList_SizeChanged(object sender, EventArgs e)
        {
        ...
        {
            string str = "select [id],[apartment] as 部门,[name] as 名称,[purpose] as 用途,[price] as 单价,[number] as 数量,[date] as 月份, [state] as 状态 from SalesFixedAssetBudget where date like '%" + dateTimePicker1.Text.Trim() + "%' and state = 1 and company = '网销区域' order by date desc";
